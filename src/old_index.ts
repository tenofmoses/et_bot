...
function createTournamentBracket(participants: Map<number, string>): TournamentBracket {
  ...
  // Determine bye player and when they should join
  let byePlayer: { id: number; name: string } | undefined;
  let byeRound = -1;

  if (playerList.length % 2 === 1) {
    // Odd number of players - last player gets bye
    const byePlayerIndex = Math.floor(playerList.length / 2) * 2; // First player not in a match
    if (byePlayerIndex < playerList.length) {
      byePlayer = playerList[byePlayerIndex];

      // Calculate when bye player should join
      // Bye player joins when there are exactly 2 players left (1 winner + bye)
      let tempPlayers = playerList.length;
      let tempRound = 0;

      while (tempPlayers > 2) {
        const matches = Math.floor(tempPlayers / 2);
        const winners = matches;
        const byes = tempPlayers % 2;

        tempPlayers = winners + byes;
        tempRound++;
      }

      // The bye player joins in the final round (when 2 players remain)
      byeRound = tempRound;

      console.log(`[DEBUG] Bye player ${byePlayer.name} will join in round ${byeRound + 1}`);
    }
  }
